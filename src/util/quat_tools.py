...
def karcher_mean(x):
    ...
    xTan = mean / np.linalg.norm(mean) 

    while True:
        sumDir = riem_log(xTan, x)
        meanDir = np.average(sumDir, axis=0)

        if (np.linalg.norm(meanDir) < tol):
            return xTan
        
        xTan = riem_exp(xTan, meanDir)




def riem_scatter(mean, x):
    scatter = np.sum(riem_log(mean, x)**2)
